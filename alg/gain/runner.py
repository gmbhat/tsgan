...
for dimension in dimensionsTested:
	print("-------------------------------------- accel_x_test third layer d/"+str(dimension)+" --------------------------------")
	string1 = "python gain.py -i accel_x_train.csv -o accel_x_imputed_train.csv --T 2 --f 1 --s 1 --t " + str(dimension) + " --testfile accel_x_test.csv"
	os.system(string1)
	string2 = "accel_x_imputed_train_d3_"+str(dimension)+".csv"
	...
	for plotWindow in windows:
		...
		imputedCSV = linecache.getline('accel_x_imputed_train_d3_'+str(dimension)+'.csv',plotWindow)
